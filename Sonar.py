...
eco2_test_active = False
test_sweep_radius = 0
test_sweep_radius_eco2 = 0
...
def draw_display_echo_2(surface, center_x, center_y, max_radius, current_anim_radius, font_obj, bg_color):
    """
    Dibuja el patrón TRX ECHO TEST (Eco-2) basado en la referencia visual:
    - 4 lóbulos diagonales (forma de 'X').
    - Gradiente de intensidad: Azul -> Verde -> Amarillo -> Rojo -> Amarillo -> Verde -> Azul.
    - Animación de barrido desde el centro.
    """
    # 1. Llenar el fondo con el color seleccionado
    surface.fill(bg_color)

    # --- DEFINICIÓN DEL PATRÓN DE COLOR ---
    # Usamos índices de la paleta de 16 colores para crear el gradiente.
    # 1-3: Azules/Cian (débil)
    # 4-6: Verdes
    # 7-9: Amarillos
    # 10-12: Naranjas
    # 13-15: Rojos (fuerte)
    # El patrón es simétrico para que el rojo quede en el centro del lóbulo.
    lobe_colors_indices = [1, 3, 5, 8, 11, 14, 11, 8, 5, 3, 1]

    # Ancho de cada franja de color dentro del lóbulo
    sector_width_deg = 3.5

    # Ángulos centrales de los 4 lóbulos diagonales (Noreste, Sureste, Suroeste, Noroeste)
    # 0 grados es el Este (derecha).
    main_angles = [-45, 45, 135, 225]

    for base_angle in main_angles:
        # Calcular el ancho total del lóbulo para centrarlo
        total_lobe_width = len(lobe_colors_indices) * sector_width_deg
        # Ángulo donde empieza a dibujarse el primer color del lóbulo
        start_angle_lobe = base_angle - (total_lobe_width / 2)

        for i, color_idx in enumerate(lobe_colors_indices):
            # Obtener el color RGB de la paleta
            color_data = COLORES_PALETTE[color_idx]
            color = (int(color_data[0]), int(color_data[1]), int(color_data[2]))

            # Calcular los ángulos de inicio y fin de este sector de color
            a_start_deg = start_angle_lobe + (i * sector_width_deg)
            a_end_deg = a_start_deg + sector_width_deg

            a_start_rad = math.radians(a_start_deg)
            a_end_rad = math.radians(a_end_deg)

            # --- GEOMETRÍA DEL SECTOR (Con borde exterior redondeado) ---
            points = [(center_x, center_y)] # Vértice central

            # Dibujar el arco exterior usando varios segmentos para que se vea curvo
            sub_steps = 5
            for j in range(sub_steps + 1):
                curr_a = a_start_rad + (a_end_rad - a_start_rad) * (j / sub_steps)
                # Usamos el radio animado actual. +2 px para evitar líneas vacías.
                px = center_x + (current_anim_radius + 2) * math.cos(curr_a)
                py = center_y + (current_anim_radius + 2) * math.sin(curr_a)
                points.append((px, py))

            # Dibujar el polígono lleno
            pygame.draw.polygon(surface, color, points)

    # --- TEXTOS EN PANTALLA (OSD) ---
    # Solo se muestran si la animación ya ha avanzado un poco
    if current_anim_radius > 60:
        def draw_osd_text(text, y_position, color_txt=(255, 255, 255)):
            # Texto blanco con sombra negra para legibilidad
            s = font_obj.render(text, True, color_txt)
            s_shadow = font_obj.render(text, True, (0, 0, 0))
            # Centrar horizontalmente
            pos_x = center_x - s.get_width() // 2
            surface.blit(s_shadow, (pos_x + 2, y_position + 2))
            surface.blit(s, (pos_x, y_position))

        # Título superior (según la imagen de referencia)
        draw_osd_text("TRX ECHO TEST", center_y - max_radius + 40)

        # Instrucción inferior
        draw_osd_text("PULSAR LA TECLA [MENU] PARA SALIR", center_y + max_radius - 60)
...
while not hecho:
    ...
    for evento in pygame.event.get():
        ...
        if eco2_test_active:
            if evento.type == pygame.KEYDOWN and evento.key == pygame.K_m:
                eco2_test_active = False
                print("INFO: Eco-2 Test desactivado.")
            continue

        # Entrada con tecla E (solo si no está activo ningún test)
        if evento.type == pygame.KEYDOWN and evento.key == pygame.K_e:
            if not test_active and not panel_test_active and not color_test_active and not pattern_test_active and not sio_test_active and not eco1_test_active and not eco2_test_active:
                menu.active = False
                ...
                if test_opt == 'UNA VEZ':
                    ...
                    eco2_test_active = True
                    test_sweep_radius_eco2 = 0
                    print(f"INFO: Eco-2 Test activado.")
    ...
    if test_active:
        ...
        # Animación del barrido para el test
        expansion_speed = 12
        if test_sweep_radius < display_radius_pixels:
            test_sweep_radius += expansion_speed
        else:
            test_sweep_radius = display_radius_pixels

        # Dibujar pantalla de test (Eco-2 / TRX ECHO TEST)
        # Usamos bg_color explícito
        draw_display_echo_2(
            pantalla,
            circle_center_x, circle_center_y,
            display_radius_pixels, test_sweep_radius,
            font_test, current_colors["BACKGROUND"]
        )
    ...
    elif eco2_test_active:
        if sonar_ping_sound: sonar_ping_sound.stop()

        # Animación Eco-2
        expansion_speed = 12
        if test_sweep_radius_eco2 < display_radius_pixels:
            test_sweep_radius_eco2 += expansion_speed
        else:
            test_sweep_radius_eco2 = display_radius_pixels

        draw_display_echo_2(
            pantalla,
            circle_center_x, circle_center_y,
            display_radius_pixels, test_sweep_radius_eco2,
            font_test, current_colors["BACKGROUND"]
        )
    else:
