...
def draw_display_echo_test(screen, center_x, center_y, radius, font_mono):
    """
    Dibuja el patrón de prueba de colores (Color Test) dentro del círculo del sonar.
    Omite el color negro (índice 0) para mostrar solo la paleta activa.
    """
    # Usamos 16 sectores para completar el círculo geométricamente
    num_sectors = 16
    angle_step = 360 / num_sectors

    # Offset para que el patrón empiece en una posición estética (ej. arriba)
    start_angle_offset = -90

    for i in range(num_sectors):
        # --- CORRECCIÓN DE PALETA ---
        # El índice 0 es negro (fondo). Para el test, queremos ver los colores de señal.
        # Mapeamos los 16 sectores a los 15 colores visibles (índices 1 al 15).
        # Usamos modulo 15 y sumamos 1.
        color_idx = (i % 15) + 1

        # Obtener color del array global numpy
        color_data = COLORES_PALETTE[color_idx]
        color_rgb = tuple(map(int, color_data)) # Convertir a tupla int para Pygame

        # Calcular ángulos en radianes para este sector
        angle_start = math.radians(start_angle_offset + (i * angle_step))
        angle_end   = math.radians(start_angle_offset + ((i + 1) * angle_step))

        # Definir los 3 puntos del triángulo (sector circular)
        p1 = (center_x, center_y) # Centro

        # Usamos radius + 2 para asegurar que cubra bien el borde sin dejar huecos negros por redondeo
        r_draw = radius + 1

        # Punto exterior inicio arco
        p2 = (center_x + r_draw * math.cos(angle_start),
              center_y + r_draw * math.sin(angle_start))

        # Punto exterior fin arco
        p3 = (center_x + r_draw * math.cos(angle_end),
              center_y + r_draw * math.sin(angle_end))

        # Dibujar el sector sólido
        pygame.draw.polygon(screen, color_rgb, [p1, p2, p3])

    # --- TEXTOS DE INFORMACIÓN (OSD) ---
    # Se dibujan encima de los colores con un fondo sombreado para que se lean bien

    def draw_text_with_shadow(text, y_offset, color=(255, 255, 255)):
        surf = font_mono.render(text, True, color)
        bg_surf = font_mono.render(text, True, (0, 0, 0)) # Sombra negra

        # Posición centrada horizontalmente, con offset vertical desde el centro del sonar
        pos_x = center_x - surf.get_width() // 2
        pos_y = center_y + y_offset

        # Dibujar sombra desplazada y luego texto
        screen.blit(bg_surf, (pos_x + 2, pos_y + 2))
        screen.blit(surf, (pos_x, pos_y))

    # 1. Título Superior (Dentro del círculo, parte superior)
    draw_text_with_shadow("DISPLAY ECHO TEST", -radius + 60)

    # 2. Indicador TX OFF (Importante en modo test)
    # Lo colocamos un poco a la derecha y arriba, como en las pantallas reales
    tx_surf = font_mono.render("TX OFF", True, (100, 255, 255)) # Cian
    tx_bg = font_mono.render("TX OFF", True, (0, 0, 0))
    tx_x = center_x + (radius * 0.4)
    tx_y = center_y - (radius * 0.5)
    screen.blit(tx_bg, (tx_x + 2, tx_y + 2))
    screen.blit(tx_surf, (tx_x, tx_y))

    # 3. Instrucción de salida (Parte inferior)
    draw_text_with_shadow("PULSAR LA TECLA [MENU] PARA SALIR", radius - 60)
