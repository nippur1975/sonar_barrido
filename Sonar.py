...
COLORES_PALETTE = np.array([
    ...
    [255, 200, 0],     # 12
    [255, 140, 0],     # 13: Orange
    [255, 80, 0],      # 14: Dark Orange
    [255, 0, 0],       # 15: Red
    [200, 0, 0],       # 16: Dark Red
    [178, 34, 34]      # 17: Brick Red (Rojo Ladrillo - Max Intensity)
], dtype=np.uint8)
...
class SonarEchoSimulator:
    ...
    def render_sweep(self, current_sweep_radius_px, sweep_speed_px_per_frame, noise_limit_level=0, color_erase_level=0):
        ...
        if np.any(mascara):
            ...
            # Mapear a colores (Range 0-17 now)
            indices = np.clip(valores, 0, 17).astype(int)

            # Aplicar Anular Color (Color Erase)
            # Suprime los índices de color por debajo de un umbral
            if color_erase_level > 0:
                # Mapa de umbrales para niveles 1-10
                # Nivel 1: Todos (Umbral 1, elimina solo 0)
                # Nivel 9: Naranja(13)+
                # Nivel 10: Solo Rojo Ladrillo (Umbral 17, elimina 0-16)
                erase_thresholds = [0, 1, 3, 5, 7, 9, 11, 13, 15, 13, 17]
                # Ajuste fino manual para cumplir requerimiento:
                # Nivel 9 -> threshold 13 (Deja 13,14,15,16,17 -> Naranjas y Rojos)
                # Nivel 10 -> threshold 17 (Deja solo 17 -> Rojo Ladrillo)
                if color_erase_level == 10:
                    thr = 17
                elif color_erase_level == 9:
                    thr = 13
                elif color_erase_level < len(erase_thresholds):
