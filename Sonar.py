...
def draw_display_echo_2(surface, center_x, center_y, max_radius, current_anim_radius, font_obj, bg_color):
    """
    Dibuja el patrón TRX ECHO TEST (Eco-2): 4 lóbulos simétricos con gradientes de intensidad.
    Verifica la tarjeta PSW y la función TVG visualmente.
    """
    # 1. Llenar el fondo con el color seleccionado
    # 1. Llenar el fondo con el color seleccionado (clipping circular)
    pygame.draw.circle(surface, bg_color, (center_x, center_y), max_radius)

    # Definición del patrón de intensidad para un lóbulo.
    # Indices actualizados: Amarillo(9)->Rojo(13)->Oscuro(15)->Rojo(13)->Amarillo(9)
    lobe_colors_indices = [9, 13, 15, 15, 15, 13, 9]

    # Ancho angular de cada sub-sector del lóbulo
    sector_width_deg = 4

    # Direcciones principales de los 4 lóbulos (Arriba, Derecha, Abajo, Izquierda)
    # En grados matemáticos (0 = Este): Norte(-90), Este(0), Sur(90), Oeste(180)
    main_angles = [-90, 0, 90, 180]
    ...
    if current_anim_radius > 50:
        def draw_text_centered(text, y_offset, color_txt=(255, 255, 255)):
        ...
        draw_text_centered("TRX ECHO TEST", -max_radius + 60)

        # Indicador TX OFF
        tx_surf = font_obj.render("TX OFF", True, (100, 255, 255))
        tx_bg = font_obj.render("TX OFF", True, (0, 0, 0))
        surface.blit(tx_bg, (center_x + max_radius*0.4 + 2, center_y - max_radius*0.5 + 2))
        surface.blit(tx_surf, (center_x + max_radius*0.4, center_y - max_radius*0.5))

        draw_text_centered("PULSAR LA TECLA [MENU] PARA SALIR", max_radius - 60)
...
def draw_display_echo_test(surface, center_x, center_y, max_radius, current_anim_radius, font_obj, config, bg_color):
    ...
    try:
        anular_color_val = int(config.get('anular_color', 0))
...
while not hecho:
    ...
    for evento in pygame.event.get():
        ...
        if eco2_test_active:
            if evento.type == pygame.KEYDOWN and evento.key == pygame.K_m:
                eco2_test_active = False
                # Reiniciar sistema al salir de Eco-2 (Condición de arranque)
                test_active = True
                test_mode = "single"
                test_start_time = time.time()
                print("INFO: Eco-2 finalizado. Sistema reiniciando (Self Test).")
            continue
