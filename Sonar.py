...
def handle_key_events(event_key, circle_center_x_param, circle_center_y_param, display_radius_pixels_param, s_max_current_range_param, heading_adjust_param=0):
    ...
    if event_key == pygame.K_y:
        ...
        if ui_state["show_plus_cursor"]:
            ...
            if current_ship_lat_deg is not None and current_ship_lon_deg is not None:
                ...
                # Effective heading for marker creation considers the adjustment
                effective_heading_for_calc = (current_ship_heading - heading_adjust_param) % 360
                true_bearing_deg = (visual_bearing_deg + effective_heading_for_calc) % 360
...
def update_marker_screen_positions(targets, ship_lat, ship_lon, ship_hdg_deg, 
    ...
    heading_adjust: Additional rotation to apply to the visual bearing (menu 'ajuste_proa').
    """
    ...
    for marker in targets:
        if marker['mode'] == 'geo':
            if ship_point and marker['geo_pos']:
                ...
                # Apply heading_adjust here: If adjust is +10, echo moves +10 (Clockwise)
                relative_bearing_deg = (true_bearing_to_target_deg - ship_hdg_deg + heading_adjust + 360) % 360
...
while not hecho:
    ...
    heading_adj = menu.options.get('ajuste_proa', 0)
    ...
    if not menu.active and dist_to_center <= display_radius_pixels:
        ...
        if display_radius_pixels > 0:
            ...
            if current_ship_lat_deg is not None and current_ship_lon_deg is not None:
                ...
                # SCREEN UP corresponds to Ship Heading - Heading Adjust.
                # True Bearing = Screen Bearing + (Ship Heading - Adjust) ?
                # If Adjust +10 moves echo Right (CW), then Screen 10 = True 0 (Rel).
                # So Screen = True_Rel + Adjust.
                # True_Rel = Screen - Adjust.
                # True_Abs = True_Rel + Heading = Screen - Adjust + Heading.

                true_bearing_deg = (bearing_deg_normalized - heading_adj + current_ship_heading + 360) % 360
    ...
    # Usar current_ship_heading para obtener el rumbo relativo REAL
    pos_rel_cardumen = cardumen_simulado.get_posicion_relativa_barco(
        ...
        current_ship_heading, # Usar current_ship_heading
        datos_nmea_disponibles=nmea_para_cardumen
    )

    # Aplicar ajuste de proa al rumbo relativo para la visualización
    # Si ajuste es +10, el eco se mueve +10 grados (horario) en la pantalla.
    pos_rel_cardumen['rumbo_relativo_deg'] = (pos_rel_cardumen['rumbo_relativo_deg'] + heading_adj) % 360
        ...
        if not menu.active:
            ...
            if evento.type == pygame.KEYDOWN:
                ...
                handle_key_events(evento.key, 
                                  ...
                                  heading_adjust_param=heading_adj)
        ...
        for marker in target_markers:
            if marker['mode'] == 'screen' and marker.get('screen_bearing_rad') is not None \
               and marker.get('initial_distance_meters') is not None: # Now using initial_distance_meters
                ...
                # Adjust formula: True = Screen - Adjust + Heading
                true_marker_bearing_deg = (current_ship_heading + screen_bearing_deg - heading_adj + 360) % 360
