...
def draw_display_echo_test(screen, font_mono):
    # Fondo Azul
    screen.fill(BLUE_FULL)
    w, h = screen.get_size()
    center_x, center_y = w // 2, h // 2
    # Radio un poco menor para que quepa bien
    radius = min(w, h) // 2 - 40

    # Función helper para dibujar sectores sólidos
    # start_angle_deg y end_angle_deg en grados matemáticos estándar (0=Este, 90=Norte)
    def draw_sector(surface, color, center, r, start_angle_math, end_angle_math):
        points = [center]
        # Pasos para suavidad
        steps = int(abs(end_angle_math - start_angle_math))
        if steps < 1: steps = 1
        for i in range(steps + 1):
            angle = math.radians(start_angle_math + i * (end_angle_math - start_angle_math) / steps)
            # Math: x = cos, y = -sin (Pygame Y invertida)
            x = center[0] + r * math.cos(angle)
            y = center[1] - r * math.sin(angle)
            points.append((x, y))
        pygame.draw.polygon(surface, color, points)

    # Sector Rojo (Top): ~330° a 30° Compás
    # Compás 330° (Noroeste) -> Math 90° + 30° = 120°
    # Compás 30° (Noreste) -> Math 90° - 30° = 60°
    # Rango Matemático: 60° a 120°
    draw_sector(screen, RED_FULL, (center_x, center_y), radius, 60, 120)

    # Sector Amarillo (Lado Izquierdo/Oeste): ~270° a 300° Compás
    # Compás 270° (Oeste) -> Math 180°
    # Compás 300° (Oeste-Noroeste) -> Math 90° + 60° = 150°
    # Rango Matemático: 150° a 180°
    YELLOW = (255, 255, 0)
    draw_sector(screen, YELLOW, (center_x, center_y), radius, 150, 180)

    # Texto
    # "DISPLAY ECHO TEST" centrado
    title_surf = font_mono.render("DISPLAY ECHO TEST", True, WHITE_FULL)
    screen.blit(title_surf, (center_x - title_surf.get_width() // 2, center_y - 10))

    # "PULSAR LA TECLA [MENU] PARA SALIR" abajo
    exit_surf = font_mono.render("PULSAR LA TECLA [MENU] PARA SALIR", True, WHITE_FULL)
    screen.blit(exit_surf, (center_x - exit_surf.get_width() // 2, h - 50))
...
while not hecho:
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.KEYDOWN and evento.key == pygame.K_e:
            if not test_active and not panel_test_active and not color_test_active and not pattern_test_active and not sio_test_active:
    ...
    if test_active:
        ...
        # Usar la nueva función de dibujo para "Display Echo Test"
        draw_display_echo_test(pantalla, font_test)
