...
# --- PALETA FURUNO CSH-5L (Saturada) ---
COLORES_PALETTE = np.array([
    [0, 0, 0],         # 0
    [0, 5, 25],        # 1: Fondo A
    [0, 20, 60],       # 2: Fondo B
    [0, 40, 100],      # 3: Azul oscuro
    [0, 70, 160],      # 4: Azul medio
    [0, 120, 220],     # 5: Azul claro
    [0, 180, 230],     # 6: Cian
    [0, 220, 180],     # 7: Turquesa
    [40, 220, 40],     # 8: Verde oscuro
    [100, 255, 0],     # 9: Verde Lima
    [180, 255, 0],     # 10: Lima-Amarillo
    [255, 255, 0],     # 11: Amarillo
    [255, 200, 0],     # 12: Oro
    [255, 140, 0],     # 13: Naranja
    [255, 80, 0],      # 14: Naranja Rojizo
    [255, 0, 0],       # 15: Rojo
    [200, 0, 0]        # 16: Rojo Sangre (Núcleo)
], dtype=np.uint8)

def generar_eco_solido(ancho_a, ancho_r, seed):
    np.random.seed(seed)

    margen_a = int(ancho_a * 1.2)
    margen_r = int(ancho_r * 1.2)

    a = np.arange(-margen_a, margen_a)
    r = np.arange(-margen_r, margen_r)
    aa, rr = np.meshgrid(a, r)

    if ancho_a <= 0: ancho_a = 1
    if ancho_r <= 0: ancho_r = 1

    # Gaussiana plana para núcleo sólido
    dist_norm = (aa / (ancho_a * 0.5))**4 + (rr / (ancho_r * 0.5))**4
    densidad = np.exp(-dist_norm * 0.8)

    ruido = np.random.uniform(0.5, 1.2, densidad.shape)

    # Intensidad alta para saturar
    eco = densidad * ruido * 40.0

    # Relleno de huecos en el centro
    nucleo_fuerte = densidad > 0.6
    eco[nucleo_fuerte] += 10

    return np.clip(eco, 0, 16).T
...
class SonarEchoSimulator:
    ...
    def inject_echo(self, dist_px, angulo_deg, grosor_fisico_m, ancho_fisico_m, rango_actual_m, intensity_factor=1.0, seed=123):
        ...
        # Generar mancha
        mancha = generar_eco_solido(ancho_a_buffer, ancho_r_buffer, seed)
    ...
    def render_sweep(self, current_sweep_radius_px, sweep_speed_px_per_frame, noise_limit_level=0, color_erase_level=0):
        ...
        if np.any(mascara):
            ...
            # Filtro de limpieza para eliminar ruido intermedio
            valores[(valores > 2.5) & (valores < 4.0)] = 0
            ...
            # Mapear a colores (Range 0-16 now)
            indices = np.clip(valores, 0, 16).astype(int)

            # Aplicar Anular Color (Color Erase)
            # Suprime los índices de color por debajo de un umbral
            if color_erase_level > 0:
                # Mapa de umbrales para niveles 1-10 (Paleta 0-16)
                # Adjusted for 16 colors
                erase_thresholds = [0, 1, 3, 5, 7, 9, 10, 11, 13, 14, 16]
    ...
    def _dibujar_scanline(self, x, config, datos_cardumen=None): 
                ...
                for y_p in range(max(0, y_pez_sup), min(y_pez_inf, y_fondo)): # y_fondo limita para no dibujar debajo de la tierra 
                    if 0 <= y_p < self.height: 
                        ...
                        if random.random() < (0.3 + intensidad_horizontal * 0.5): 
                            ...
                            # Núcleo del cardumen (Indices 12-16: Rojo->Rojo Sangre)
                            if intensidad_horizontal > 0.5 and rand_val > 0.3: 
                                col = COLORES_ECO[16] # Rojo Sangre (Núcleo)
                            elif intensidad_horizontal > 0.3 and rand_val > 0.3: 
                                col = COLORES_ECO[14] # Naranja Rojizo
                            elif rand_val > 0.5:
                                col = COLORES_ECO[11] # Amarillo
                            else: 
                                col = COLORES_ECO[8] # Verde Oscuro
        ...
        for i in range(grosor): 
            ...
            if 0 <= y_draw < self.height: 
                ...
                # Mapear 0.0-1.0 a indices 1-16
                color_index = 1 + int(displayed_intensity * 15)
                color_index = max(1, min(16, color_index))
        ...
        if y_quilla > 0: 
            for y in range(0, min(y_quilla, self.height)): 
                if random.random() < 0.9:  
                    ...
                    if dist_relativa < 0.3: color = COLORES_ECO[16] # Núcleo duro
                    elif dist_relativa < 0.6: color = COLORES_ECO[14] # Naranja Rojizo
                    elif dist_relativa < 0.8: color = COLORES_ECO[12] # Oro
                    else: color = COLORES_ECO[9] # Verde Lima
