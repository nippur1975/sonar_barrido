...
class SonarEchoSimulator:
    ...
    def render_sweep(self, current_sweep_radius_px, sweep_speed_px_per_frame, noise_limit_level=0):
        ...
        if np.any(mascara):
            ...
            # Level 2 -> Threshold ~2.4 (Elimina fondo 0-2.5)
            # Level 8 -> Threshold ~9.6 (Elimina verdes)
            # Level 10 -> Threshold 11.0 (Máximo, respeta el eco rojo/naranja)
            if noise_limit_level > 0:
                threshold = min(noise_limit_level * 1.2, 11.0)
...
while not hecho:
    ...
    if menu.options.get("transmision") == "ON":
        ...
        echo_simulator.render_sweep(int(current_sweep_radius_pixels), int(sweep_increment_ppf) + 1, noise_limit_level=limitar_ruido_val)
