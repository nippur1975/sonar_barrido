...
while not hecho:
    ...
    if serial_port_available and current_ship_lat_deg is not None and current_ship_lon_deg is not None:
        if not cardumen_posicion_geografica_inicializada:
            ...
            true_bearing_to_cardumen_deg = (effective_heading + bearing_sim_deg + 360) % 360
            ...
            try:
                ...
                print(f"INFO: Cardumen inicializado geográficamente en Lat: {cardumen_simulado.lat:.4f}, Lon: {cardumen_simulado.lon:.4f} (Dist: {distancia_sim_m:.1f}m, Bearing: {true_bearing_to_cardumen_deg:.1f}° from ship at {current_ship_lat_deg:.4f},{current_ship_lon_deg:.4f} heading {effective_heading:.1f}°)")
    ...
    # Usar effective_heading (que incluye ajuste de proa)
    pos_rel_cardumen = cardumen_simulado.get_posicion_relativa_barco(
        ...
        effective_heading, # Usar effective_heading
    ...
    draw_compass_rose(pantalla, center_x, center_y, main_radius, font, current_colors["COMPASS_ROSE"], effective_heading)
