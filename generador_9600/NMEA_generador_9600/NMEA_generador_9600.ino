...
void updateSpeed() {
  ...
  if (potVal < 230 && speedKn > 0.5 && estabaAdelante) speedKn = 0.0;
  estabaAdelante = (potVal > 300);

  float targetSpeed = 0.0;
  if (potVal > 300) {
    if (potMax - 300 == 0) return;
    float ratio = float(potVal - 300) / float(potMax - 300);
    ratio = constrain(ratio, 0.0, 1.0);
    // Avance hasta 13.3 kn con compensación de mar ±0.2 kn
    float baseSpeed = 13.3 * (exp(3.0 * ratio) - 1.0) / (exp(3.0) - 1.0);
    float waveComp = random(-20, 21) * 0.01;  // ±0.2 nudos
    targetSpeed = baseSpeed + waveComp;
  } else if (potVal >= 230 && potVal <= 300) {
    ...
    if (speedKn > 0.1) {
      ...
      float ratio = float(230 - potVal) / 230.0;
      ratio = constrain(ratio, 0.0, 1.0);
      targetSpeed = -5.0 * log(1 + 9 * ratio) / log(10);
    }
  }

  float maxDelta = (interval / 60000.0) * 13.3;
...
void updateDepth() {
  // Normaliza el heading
  float h = fmod(heading + 360.0, 360.0);
  float d = min(distanceNM, 10.0); // solo consideramos hasta 10 millas

  if ((h >= 45 && h <= 135) || (h > 135 && h < 180)) {
    // Este, Noreste, Sureste
    if (d <= 5.0) {
      depth = 200.0 - (180.0 * (d / 5.0));  // de 200 a 20
    } else {
      depth = 20.0 - (15.0 * min((d - 5.0) / 5.0, 1.0));  // de 20 a 5
    }
  } else if ((h >= 225 && h <= 315)) {
    // Oeste, Noroeste, Suroeste
    depth = 140.0 + (60.0 * min(d / 10.0, 1.0)); // de 140 a 200
  } else {
    // Otros rumbos (Norte, Sur, etc.)
    depth = 75.0 + 25.0 * sin(millis() / 10000.0);  // default
  }
}
